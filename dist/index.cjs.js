...
const ModuleRegistryContext = react.createContext(undefined);
const createGlobalStore = (modules) => {
    const Provider = ({ children }) => {
        ...
        modules.forEach((mod) => {
            ...
            const [state, dispatch] = react.useReducer(reducer, mod.states);
        ...
        return (jsxRuntime.jsx(ModuleRegistryContext.Provider, { value: registry, children: children }));
    };
    return { Provider };
};
...
// core/useModule.ts
const useModule = () => {
    const registry = react.useContext(ModuleRegistryContext);
    if (!registry)
